/*
  # Final Fix: Branding and Leads Enhancements
...
-- Add constraints using DO blocks (NO IF NOT EXISTS here!)
DO $$
BEGIN
  ...
  -- Add source platform constraint
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
CREATE INDEX IF NOT EXISTS idx_leads_source_created ON leads (source_platform, created_at DESC);
...
CREATE POLICY "Anyone can read branding" ON company_branding
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Only admins can modify branding" ON company_branding
...
CREATE POLICY "Anyone can read employee profiles" ON employee_profiles
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Only admins can modify employee profiles" ON employee_profiles
  FOR ALL TO authenticated 
  USING (
    EXISTS (
      SELECT 1 FROM app_users au 
